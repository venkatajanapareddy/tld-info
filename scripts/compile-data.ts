...
async function fetchIANATLDs(): Promise<string[]> {
  ...
  try {
    ...
    console.log(`Fetched ${tlds.length} TLDs from IANA.`);
    return tlds;
  } catch (error) {
    console.error(
 ...
 */
function enrichTLDData(rawTld: string): TLDInfoData {
...
async function main() {
  console.log('Starting TLD data compilation...');

  const rawTlds = await fetchIANATLDs();
  const enrichedTlds: TLDInfoData[] = rawTlds.map(enrichTLDData);

  // Create the map for faster lookups
  const tldInfoMap: TLDInfoMap = {};
  enrichedTlds.forEach((tldInfo) => {
    tldInfoMap[tldInfo.tld] = tldInfo;
  });

  // Create the simple list of TLD strings
  const tldList: TLDString[] = enrichedTlds.map((info) => info.tld);

  // Generate the TypeScript data file content
  const tsContent = `// This file is auto-generated by scripts/compile-data.ts
...
`;
  ...
  // Ensure dist directory exists for JSON output
  ...
  console.log(`Successfully wrote TypeScript data to ${OUTPUT_TS_PATH}`);

  // Write JSON data for direct consumption
  fs.writeFileSync(
    OUTPUT_JSON_PATH,
    JSON.stringify(enrichedTlds, null, 2),
    'utf8'
  );
  console.log(`Successfully wrote JSON data to ${OUTPUT_JSON_PATH}`);

  console.log('TLD data compilation finished.');
}

main().catch((error) => {
  console.error(
    'Error during TLD data compilation:',
